...
namespace GexpoTechCMS.Controllers
{
    public class VideosController : Controller
    {
        ...
        public VideosController(DBConnection context, ILogger<VideosController> logger, IOptions<SystemConfiguration> systemConfiguration, IOptions<AppMessages> appMessages, IDetectionService detectionService, IHttpContextAccessor accessor, SessionManager sessionManager, IWebHostEnvironment hostingEnvironment)
        {
            _context = context;
            _logger = logger;
        ...
        public async Task<IActionResult> Index([FromQuery(Name = "p")] string p = "1", [FromQuery(Name = "s")] string s = "12") 
        {
            ...
            ViewBag.TotalRecords = _context.Videos.Count(s => s.Status == 1);

            var VideosModel = _context.Videos.Where(s => s.Status == 1).OrderByDescending(s => s.ID).Skip(PageSkip).Take(PageSize);

            return View(await VideosModel.ToListAsync());
        }
