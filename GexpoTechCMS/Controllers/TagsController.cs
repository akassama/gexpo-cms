...
namespace NgoExpoApp.Controllers
{
    public class TagsController : Controller
    {
        ...
        public TagsController(DBConnection context, ILogger<TagsController> logger, IOptions<SystemConfiguration> systemConfiguration, IDetectionService detectionService, IHttpContextAccessor accessor, SessionManager sessionManager)
        {
            _context = context;
            _logger = logger;
        ...
        public async Task<IActionResult> Index(string id, [FromQuery(Name = "p")] string p = "1", [FromQuery(Name = "s")] string s = "12")
        {
            ...
            //if posts disabled, tags also disabled
            ...
            functions.LogVisit(id, "Tags", VisitorIP, _detectionService.Browser.Name.ToString(), _detectionService.Device.Type.ToString());
            ...
            ViewBag.TotalRecords = _context.Posts.Count(s => s.Status == 1 && (s.Title.Contains(id) || s.ShortDescription.Contains(id) || s.PostTags.Contains(id)));

            var postsModel = _context.Posts.Where(s => s.Status == 1 && (s.Title.Contains(id) || s.ShortDescription.Contains(id) || s.PostTags.Contains(id))).OrderByDescending(s => s.ID).Skip(PageSkip).Take(PageSize);
