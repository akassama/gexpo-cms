...
*/
	...
	*/
	$('.edit-resource').on('click', function (e) {
		...
		var documentDescription = $(this).data('desc');
		var documentStatus = $(this).data('status');


		//check if main inputs are empty
		if (isEmpty(documentID) || isEmpty(documentName)) {
			alert("missng required data values");
			return false;
		}
		...
		$(".document-description").val(documentDescription);

		//set modal select options
		if ($.trim(documentStatus) == '1') {
			$('#DocumentStatus').find(".document-published").prop('selected', true);
		}
		else {
			$('#DocumentStatus').find(".document-pending").prop('selected', true);
		}
	...
	*/
	$('.edit-video').on('click', function (e) {
		...
		//check if main inputs are empty
		if (isEmpty(videoID) || isEmpty(videoName)) {
			alert("missng required data values");
			return false;
		}
		...
		$(".video-link").val(videoLink);

		//set modal select options
		if ($.trim(videoStatus) == '1') {
			$('#VideoStatus').find(".video-published").prop('selected', true);
		}
		else {
			$('#VideoStatus').find(".video-pending").prop('selected', true);
		}
